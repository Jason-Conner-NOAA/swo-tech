...
Observations of sex-specific length distributions for designated species encountered within a catch sample are collected by: 
...
Traditionally, a high degree of independence has been conferred to each Deck Lead regarding sub-sample selection, and these analyses have been provided as guidelines. 
Thus there is some inherent variability in the application of these guidelines, as there are multiple Deck Leads assigned to multiple vessels and legs for a cruise, with an inclination to sample more than less.
...
For the analysis we conducted, RACEBASE was queried using the R package sumfish (https://github.com/afsc-gap-products/sumfish). 
...
Length frequency samples collected by the AFSC bottom trawl surveys are expanded by catch-per-unit-effort (CPUE) and stratum area to obtain estimates of population abundance at length (i.e., design-based expansion). 
...
To summarize uncertainty across bootstrap replicates of ESS, we calculated ISS as the harmonic mean:
...
In general, the mean of the regional, replicated sex-specific ESS for length compositions increased as the number of collected lengths for which sex was determined increased (Figures \@ref(fig:ai-length) - \@ref(fig:goa-length)). 
...
These exceptions include arrowtooth flounder in the eastern Bering Sea (Figure \@ref(fig:ebs-length)) and Gulf of Alaska (Figure \@ref(fig:goa-length)), and walleye pollock in the eastern Bering Sea (Figure \@ref(fig:ebs-length)). 
...
Results of the sex-specific ISS for length composition mimicked the results of the replicated ESS for each region; as the sexed sub-sampling level increased the ISS increased (Figures \@ref(fig:ai-iss) - \@ref(fig:goa-iss)). 
...
Sex-specific ISS for length frequency ranged from around 50 to 1,200 for the Aleutian Islands bottom trawl survey (Figure \@ref(fig:ai-iss)), 500 to 3,000 for the eastern Bering Sea bottom trawl survey (Figure \@ref(fig:ebs-iss)), and 50 to 3,000 for the Gulf of Alaska bottom trawl survey (Figure \@ref(fig:goa-iss)). 
...
Compared to the base ISS (the case using the full historical data set), the proportion of the base ISS for each sexed sub-sampling case was similar for females and males, but were different across species and survey years (Figures \@ref(fig:ai-iss-prop) - \@ref(fig:goa-iss-prop)). 
...
The most notable exception was for eastern Bering Sea walleye pollock in each year of the survey, whose proportion of the base ISS for each of the sexed sub-sampling cases investigated ranged from 0.25 to 0.5 (Figure \@ref(fig:ebs-iss-prop)). 
