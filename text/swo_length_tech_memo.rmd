...
where ## is the number of hauls, $L_infty$ is the catch per unit effort of numbers caught within a haul-h, ## is the catch (in numbers) in haul-h, and ## is the effort in haul-h, which is computed as the net width multiplied by the time on bottom, or, the area swept by the haul (in km^2^). Next, the ratio of catch per unit effort among hauls (##) is computed by:
