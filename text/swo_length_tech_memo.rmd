...
The species with the greatest average total number of lengths sampled varied by the regional-specific surveys (Tables \@ref(tab:tbl1) - \@ref(tab:tbl3)). 
...
The species most frequently sampled for lengths across all the surveys was walleye pollock in the eastern Bering Sea survey (Table \@ref(tab:tbl2)), with nearly 50% more samples in some years as the next most frequently sampled species (arrowtooth flounder in the Gulf of Alaska, Table  \@ref(tab:tbl3)).
...
These exceptions include arrowtooth flounder in the Gulf of Alaska (\@ref(fig:goa-length-flats)) and eastern Bering Sea (\@ref(fig:bs-length)), and walleye pollock in the eastern Bering Sea (\@ref(fig:bs-length)), with the most extreme difference resulting for eastern Bering Sea walleye pollock. 
...
knitr::kable(vroom::vroom(here::here('tables', 'avg_saved_range.csv')), caption = "Range of average reduction in sexed length frequency samples for the sex sub-sampling cases investigated from the most recent three surveys.", align = c('lccc'))
```
