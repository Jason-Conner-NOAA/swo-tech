...
Length frequency data are collected and sex is subsequently determined from a subset of these collected data during Alaska Fishery Science Center bottom trawl surveys. This is done so that sex-specific length composition data are available for assessment of groundfish stocks. 
...
The working group developed a bootstrap-simulation approach that determined the amount of uncertainty gained by reducing the amount of length frequency data for which sex was subsequently determined. The analysis was completed during the spring of 2022. 
It was shown that for the majority of species the length composition input sample size was reduced by 25% or less compared to the input sample size for the full historical data sets for sub-sampling levels of 100 fish per haul for which sex was subsequently determined. 
...
Two of the primary results from this analysis were: (1) with an increasing level of sub-sampling (i.e., fewer samples) of length frequency to determine sex the uncertainty in the resulting sex-specific length composition increases, and (2) the uncertainty in the total length composition (which is the result of the sum of the sex-specific length population estimates) is robust to the sub-sampling for sex of the length frequency data. 
...
Some of these flatfish assessments use the sex-specific length frequency samples in addition to the sex-specific specimen collections (which include information on weight and age for an individual fish) to estimate growth internally and fit age composition data in the assessment model through conditional age-at-length methods [e.g., @Turnock2017]. Additionally sex-specific, length-based selectivity is often estimated in these assessments to account for differences in availability to the survey. Therefore our estimates of spawning stock biomass are highly dependent on our understanding of trends in female length frequency. 
In addition, recent research has indicated that growth of certain flatfish stocks may be regionally-specific and thus spatially-explicit stock assessments have been developed to account for different growth rates in different regions [@Mcgilliard2017; @Bryan2021]. Thus, the reduction in the sex-specific input sample size for length composition based on the sexed sub-sampling level may have a greater influence on the results of these assessments. 
