...
The database (i.e., RACEBASE) was queried based on functions used in the sumfish package in Program R (https://github.com/afsc-gap-products/sumfish *(Add Conner reference)*). 
...
In this bootstrap-simulation the underlying length composition derived from the historical bottom trawl surveys was treated as the observed proportions $O_c$ in equation \@ref(eg:eqn7). 
...
The average total number lengths sampled over the most recent three surveys is summarized for each survey and species of interest in Table \@ref:tbl1. 
The species with the greatest average total number of lengths sampled varied by survey. 
The top five species for the eastern Bering Sea shelf bottom trawl survey are walleye pollock, yellowfin sole, northern rock sole, flathead sole, and arrowtooth flounder; POP, northern rock sole, northern rockfish, walleye pollock, and arrowtooth flounder for the Aleutian Islands bottom trawl survey; and arrowtooth flounder, POP, walleye pollock, flathead sole, and rex sole for the Gulf of Alaska bottom trawl survey.


The number of sampled lengths per haul for species of interest are summarized in (Figures \@ref(fig:ai-samples-per-haul) - \@ref(fig:goa-samples-per-haul)). All summarize the most recent six years of data. The species with the greatest number of hauls exceeding the sampling target of 200 samples are walleye pollock, yellowfin sole, and northern rock sole in the eastern Bering Sea; POP, northrn rockfish, and walleye pollock in the Aleutian Isalnds; and POP, arrowtooth flounder, and walleye pollock in the Gulf of Alaska.


In general, for each region investigated the mean of the replicated sex-specific effective sample size for length composition increased as the number of collected lengths for which sex was determined increased (Figures \@ref(fig:goa-length) - \@ref(fig:goa-length-flats)). 
...
For nearly all the species investigated (*or all, will need to see results*), the mean of the replicated effective sample size for the total length composition (the sum of the female, male, and unsexed population at length estimates) were insensitive to the sexed subsampling cases. (*Any exceptions to previous statement here? This seems to hold for all species and surveys*)
...
Sex-specific input sample sizes for length frequency ranged from around 50 to 750 for the Aleutian Islands bottom trawl survey and 500 to 2000 for the EBS shelf bottom trawl survey. 
...
Total input sample size for length were species-specific, with values around 50 for less frequently sampled species to 1250 and 3500 for more frequently sampled species in the ALeutian Islands and eastern Bering Sea, respectively.
...
*Exceptions were…*
For a number of species and survey years the sex-specific proportion for the sexed subsampling cases were similar to the base input sample size, particularly for sexed subsampling sizes greater than 100. Notable exceptions were EBS walleye pollock, EBS Pacific cod in 2019, and EBS flathead sole in 2018. (*Everyone agree with this?*)  
In general, the proportion of the sexed subsample size of 100 was greater than 0.75 of the base input sample size (*exceptions are EBS and AI flathead sole in 2018, EBS arrowtooth flounder 2018 and 2019, EBS Pcod in 2019, and EBS walleye pollock*).
...
knitr::kable(mtcars[1:5, 1:5], caption = "Species evaluated in the bootstrap-simulation for reduction in sexed length-frequency collections. (maybe could even merge this table with one that reports the average historical length sample sizes).")
```
...
```{r length, fig.cap="Bootstrap-simulation flow chart, the steps refer to the order of operations as described in the *Bootstrap-simulation framework* section.", fig.alt="Flowchart diagram of bootstrap-simulation"}
...
knitr::include_graphics(here::here('figs', 'ess_length_bs.png'))
```
...
knitr::include_graphics(here::here('figs', 'iss_length_bs.png'))
```
...
knitr::include_graphics(here::here('figs', 'prop_iss_bs.png'))
```
