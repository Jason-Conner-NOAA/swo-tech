...
On the Alaska Fisheries Science Center bottom trawl surveys length frequency sub-samples are collected by sex so that sex-specific length composition data are available for assessment of groundfish stocks. Determining sex for the length frequency sub-samples has resulted in acute and repetitive stress injuries of survey staff, and these injuries have become increasingly common. 
In December 2021, a working group was formed (the Survey Workflow Optimization working group) to evaluate the effect of reducing the amount of length frequency sub-sample size for which sex is determined from historical levels in the bottom trawl surveys on length composition uncertainty. 
Over the spring of 2022, the working group developed a bootstrap simulation approach that determined the amount of uncertainty gained by reducing the amount of length frequency data for which sex was subsequently determined. 
It was shown that the length composition input sample size was reduced by 25% or less compared to the input sample size for the full historical data sets for sub-sampling levels of 100 fish per haul. 
Upon meeting with stock assessment authors the joint recommendation between the working group and assessment authors to the survey staff was to limit length frequency data for which sex is subsequently determined to a target of 100 subsampled fish per haul.
...
The National Oceanic and Atmospheric Administration’s (NOAA’s) Alaska Fisheries Science Center (AFSC) has conducted effort-standardized bottom trawl surveys in the eastern Bering Sea (EBS), Gulf of Alaska (GOA) and Aleutian Islands (AI) since 1982 (Stauffer 2004). 
These resource surveys comprehensively catalog the biota encountered at each sampling station. Observations of sex-specific size distributions for designated species encountered within a catch sample are collected by: 
...
(5) measuring and recording each fish length (each fish is placed on a measuring board, length is identified and the species, sex and measurement are recorded into a computer). 
...
From the stock assessment perspective, the primary concern when reducing the sampling for sex within the length frequency data collections is the impact on the uncertainty in subsequent sex-specific length composition data. 
...
To that end, we evaluated the impact of reducing sampling for sex within the length frequency data collection to answer two questions: 
(1) what is the impact of reducing sampling for sex on the uncertainty in the sex-specific length composition? And, 
(2) is there an acceptable level of increase in uncertainty in sex-specific length composition data due to sub-sampling for sex?  

# Materials and Methods 

## Survey data 

Data collection for each AFSC groundfish survey is described in a respective NOAA Technical Memorandum [EBS: @Lauth2019; AI: @vonSzalay2017; GOA: @vonSzalay2018].
Fundamental methods of length sample collection are generally synchronized between these surveys, with species-specific exceptions for minimum length sub-sample size. Survey protocols include sampling bimodal length distributions separately (e.g. many juveniles and fewer adults within one species), which are then extrapolated to the total estimated number of individuals in each length mode category for that trawl sample and aggregated. 
...
where $N_{sx,l,h,st,y}$ is the count of sampled fish, by sex-*sx* and length-*l*. 
...
The number of sexed sampled lengths per haul for species of interest are summarized in Figures \@ref(fig:ai-samples-per-haul) - \@ref(fig:goa-samples-per-haul), each of which summarize the most recent three years of data. 
...
There were notable exceptions to the sub-sampling case of 175 being similar to the base case, indicating that in the historical surveys there were a number of hauls in which more than 175 sex-specific length were measured. 
...
Two of the primary results from this analysis were: (1) with an increasing level of sub-sampling of sex-specific length frequency the uncertainty in the resulting sex-specific length composition increases, but, (2) the uncertainty in the total length composition (which is the result of the sum of the sex-specific length population estimates) is robust to the sub-sampling of the length frequency data. 
