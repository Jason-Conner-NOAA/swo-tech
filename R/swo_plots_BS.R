...
png("ess_length_bs1719.png",width=13,height=10,units="in",res=300)
print(ess_bp)
dev.off()
...
df %>% filter(year %in% 2017:2019) %>%
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) -> harm_mn_p
png("iss_length_bs1719.png",width=13,height=10,units="in",res=300)
print(harm_mn_p)
dev.off()
...
.df %>% 
  ...
  xlab("\nSubsample size case") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) -> prop_change
png("prop_iss_bs1719.png",width=13,height=10,units="in",res=300)
print(prop_change)
dev.off()
